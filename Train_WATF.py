...
parser.add_argument('--dataset_dir', default='/public/home/yan/dn4_dataset/miniimagnet/miniImageNet--ravi', help='/miniImageNet')
...
parser.add_argument('--outf', default='./results_CN/Adam_Cosine_Lr0.001_k=1_CN')
...
if __name__=='__main__':
	...
	model = FewShotNet.define_model(encoder_model=opt.encoder_model, classifier_model=opt.classifier_model, norm='batch',
			way_num=opt.way_num, shot_num=opt.shot_num, init_type='normal', use_gpu=opt.cuda,neighbor_k=opt.neighbor_k,)
	if opt.cuda :
		model = model.cuda ()
	...
	for epoch_item in range(opt.start_epoch, opt.epochs):
		...
		if epoch_item % 5 == 0:#每5epoch保存一次权重文件
